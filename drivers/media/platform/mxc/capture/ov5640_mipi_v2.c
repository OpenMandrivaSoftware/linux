...
static int __maybe_unused ov5640_suspend(struct device *dev);
static int __maybe_unused ov5640_resume(struct device *dev);
#ifdef CONFIG_OF
...
UNIVERSAL_DEV_PM_OPS(ov5640_pm_ops, ov5640_suspend, ov5640_resume, NULL);
...
{
	dev_dbg(&sensor->i2c_client->dev, "starting %s\n", __func__);
				...
				enum ov5640_mode mode)
{
	...
	retval = pm_runtime_get_sync(&sensor->i2c_client->dev);
	if (retval < 0) {
		dev_err(&sensor->i2c_client->dev, "%s: pm_runtime_get fail: %d\n",
			__func__, retval);
			    ...
			    enum ov5640_mode mode, enum ov5640_mode orig_mode)
{
	...
	if (retval < 0) {
		dev_err(dev, "%s: pm_runtime_get fail: %d\n",
			__func__, retval);
				   ...
				   struct v4l2_subdev_frame_interval *f_interval)
{
	...
	dev_err(&sensor->i2c_client->dev, "starting %s\n", __func__);
	...
	dev_err(&sensor->i2c_client->dev, "timepreframe %d/%d\n",
		timeperframe->numerator, timeperframe->denominator);
	sensor->streamcap.timeperframe = f_interval->interval;
				   ...
				   struct v4l2_subdev_frame_interval *f_interval)
{
	...
	f_interval->interval = sensor->streamcap.timeperframe;
			...
			struct v4l2_subdev_format *format)
{
	...
	if (capturemode >= 0) {
		...
		ret = ov5640_init_mode(sensor, frame_rate,
				capturemode, sensor->streamcap.capturemode);
		if (ret < 0)
			return ret;
		...
		dev_err(&client->dev, "Set format w=%d h=%d mode=%d code=0x%x colorspace=0x%x\n",
			  ...
			  struct v4l2_subdev_format *format)
{
	...
	dev_err(&client->dev, "Get format w=%d h=%d code=0x%x colorspace=0x%x\n",
			mf->width, mf->height,
		...
		struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	dev_dbg(dev, "starting %si index %d\n", __func__, fie->index);
	...
	if (fie->width == 0 || fie->height == 0 ||
	    fie->code == 0) {
		dev_err(dev, "Please assign pixel format, width and height\n");
		return -EINVAL;
	}
		...
		for (j = 0; j < (ov5640_mode_MAX + 1); j++) {
			...
			if (fie->index == (count - 1)) {
				fie->interval.denominator =
						ov5640_framerates[i];
				dev_err(dev, "%s matched frame interval %d/%d\n",
					__func__,
	...
	dev_err(dev, "%s Failed to match frame interval\n", __func__);
	return -EINVAL;
}
			...
			const struct i2c_device_id *id)
{
	...
	if (retval < 0 || chip_id_high != 0x56) {
		dev_err(dev, "Camera is not found %d %d\n", retval, chip_id_high);
		ov5640_regualtor_disable();
		clk_disable_unprepare(sensor->sensor_clk);
		return -ENODEV;
	}
	...
	if (retval < 0) {
		ov5640_regualtor_disable();
		clk_disable_unprepare(sensor->sensor_clk);
		dev_err(dev, "Camera init failed %d\n", retval);
		ov5640_power_down(sensor, 1);
		return retval;
	}
