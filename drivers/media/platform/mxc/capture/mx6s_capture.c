...
static int mx6s_csi_enable(struct mx6s_csi_dev *csi_dev)
{
	...
	if (csi_dev->csi_mipi_mode == true) {
		pr_err("configuring csi_mipi mode.\n");
...
static int mx6s_configure_csi(struct mx6s_csi_dev *csi_dev)
{
	...
	case V4L2_PIX_FMT_YUYV:
		pr_debug("pixelformat: V4L2_PIX_FMT_YUYV\n");
		if (csi_dev->csi_mipi_mode == true)
			width = pix->width;
		else
		...
		int bufnum, bool err)
{
	...
	dev_dbg(csi_dev->dev, "%s: starting", __func__);
	...
	dev_dbg(csi_dev->dev, "frame count: %d\n", csi_dev->frame_count);
...
static irqreturn_t mx6s_csi_irq_handler(int irq, void *data)
{
	...
	dev_dbg(csi_dev->dev, "%s: starting 0x%lx\n", __func__, status);
				    ...
				    struct v4l2_format *f)
{
	...
	dev_err(csi_dev->dev, "get pixelformat '%4.6s' w=%d h=%d size=%d\n",
			     ...
			     struct v4l2_streamparm *a)
{
	...
	dev_dbg(csi_dev->dev, "Starting %s type=%d\n", __func__, a->type);
	...
	ret = v4l2_subdev_call(sd, video, g_frame_interval,
			       &frame_interval);
	if (!ret) {
		cparm->timeperframe = frame_interval.interval;
		cparm->capturemode = V4L2_MODE_HIGHQUALITY;
		cparm->capability = V4L2_CAP_TIMEPERFRAME |
			V4L2_MODE_HIGHQUALITY;
	} else {
		dev_err(csi_dev->dev, "%s: get frame interval failed %d\n",
			__func__, ret);
		return ret;
	}
				...
				struct v4l2_streamparm *a)
{
	...
	memcpy(&frame_interval.interval, timeperframe,
	       sizeof(struct v4l2_fract));
	ret = v4l2_subdev_call(sd, video, s_frame_interval, &frame_interval);
	if (ret) {
		dev_err(csi_dev->dev, "%s: set frame interval failed %d\n", 
			__func__, ret);
		return ret;
	}
				 ...
				 struct v4l2_queryctrl *qc)
{
	...
	return v4l2_queryctrl(ctrl_handler, qc);
}
			      ...
			      struct v4l2_control *a)
{
	...
	struct v4l2_subdev *sd = csi_dev->sd;
	struct v4l2_ctrl_handler *ctrl_handler = sd->ctrl_handler;
	return v4l2_s_ctrl(fh, ctrl_handler, a);
}
...
static int init_icc(struct platform_device *pdev)
{
	...
	struct mx6s_csi_dev *csi_dev = container_of(v4l2_dev,
						struct mx6s_csi_dev, v4l2_dev);
	int ret;

	/* Optional interconnect request */
	csi_dev->icc_path = of_icc_get(&pdev->dev, NULL);
	if (IS_ERR(csi_dev->icc_path)) {
		ret = PTR_ERR(csi_dev->icc_path);
		if (ret == -EPROBE_DEFER)
			return ret;

		csi_dev->icc_path = NULL;
	}

	/* Force DDRC high on imx8mq: */
	csi_dev->icc_path_bw = 678900;

	icc_set_bw(csi_dev->icc_path, 0, csi_dev->icc_path_bw);
...
static int mx6s_csi_runtime_suspend(struct device *dev)
{
	...
	icc_set_bw(csi_dev->icc_path, 0, 0);
