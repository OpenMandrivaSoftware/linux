...
static int mipi_csis_s_parm(struct v4l2_subdev *mipi_sd, struct v4l2_streamparm *a)
{
	...
	if (a->type == V4L2_BUF_TYPE_VIDEO_CAPTURE &&
	    a->capture->capturemode == V4L2_CAP_TIMEPERFRAME) {
		v4l2_subdev_call(sensor_sd, video, s_frame_interval, a->parm.capture->timeperframe);
	}
	return 0;
}
...
static int mipi_csis_g_parm(struct v4l2_subdev *mipi_sd, struct v4l2_streamparm *a)
{
	...
	if (a->type == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
		v4l2_subdev_call(sensor_sd, video, g_frame_interval, &a->parm.capture->timeperframe);
