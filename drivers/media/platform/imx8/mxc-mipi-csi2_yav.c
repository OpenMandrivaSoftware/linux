...
static int mipi_csi2_clk_init(struct mxc_mipi_csi2_dev *csi2dev)
{
	...
	csi2dev->clk_clko2 = devm_clk_get(dev, "clk_clko2");
	if (IS_ERR(csi2dev->clk_clko2)) {
		dev_err(dev, "failed to get clko2 clk\n");
		return PTR_ERR(csi2dev->clk_clko2);
	}
...
static int mipi_csi2_clk_enable(struct mxc_mipi_csi2_dev *csi2dev)
{
	...
	ret = clk_prepare_enable(csi2dev->clk_clko2);
	if (ret < 0) {
		dev_err(dev, "%s, pre clk_clko2 error\n", __func__);
		return ret;
	}
	return ret;
}
...
static void mipi_csi2_clk_disable(struct mxc_mipi_csi2_dev *csi2dev)
{
	...
	clk_disable_unprepare(csi2dev->clk_clko2);
}
...
static int mipi_csis_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *a)
{
	...
	if (a->type == V4L2_BUF_TYPE_VIDEO_CAPTURE &&
	    a->parm.capture.capturemode == V4L2_CAP_TIMEPERFRAME) {
		v4l2_subdev_call(sensor_sd, video, s_frame_interval,
...
static int mipi_csis_g_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *a)
{
	...
	if (a->type == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
		v4l2_subdev_call(sensor_sd, video, g_frame_interval,
				 &a->parm.capture.timeperframe);
		a->parm.capture.capturemode = V4L2_CAP_TIMEPERFRAME;
	}
			       ...
			       struct v4l2_subdev_frame_interval *interval)
{
	...
	return v4l2_subdev_call(sensor_sd, video, s_frame_interval,
				 interval);
}
			       ...
			       struct v4l2_subdev_frame_interval *interval)
{
	...
	return v4l2_subdev_call(sensor_sd, video, g_frame_interval,
				 interval);
}
				 ...
				 struct v4l2_async_subdev *asd)
{
	...
	if (subdev == NULL)
		return -EINVAL;
	err = v4l2_ctrl_add_handler(&csi2dev->ctrl_handler,
				    csi2dev->sensor_sd->ctrl_handler,
				    NULL, true);
	if (err)
		v4l2_err(&csi2dev->v4l2_dev, "Failed to set controls");
...
static int mipi_csi2_probe(struct platform_device *pdev)
{
	...
	ret = v4l2_ctrl_handler_init(&csi2dev->ctrl_handler, 16);
	if (ret < 0) {
		dev_err(&pdev->dev, "%s--Cannot init ctrl handler (%d)\n", __func__, ret);
		return ret;
	}
