...
static int tps6105x_startup(struct tps6105x *tps6105x)
{
	...
	if (ret) {
		dev_err(&tps6105x->client->dev,
				"register read error %d, retrying..\n", ret);
		msleep(10);
		ret = regmap_read(tps6105x->regmap, TPS6105X_REG_0, &regval);
		if (ret)
			return ret;
...
#ifdef CONFIG_OF
static struct of_device_id tps6105x_dt_match[] = {
	{ .compatible = "ti,tps61050", .data = NULL },
	{ .compatible = "ti,tps61052", .data = NULL },
	{},
};

static struct tps6105x_platform_data *
tps6105x_i2c_parse_dt_pdata(struct device *dev)
{
	int ret;
	struct tps6105x_platform_data *pd;
	...
	ret = of_property_read_u32(of_node, "operating-mode", &mode);
	if (ret != 0) {
		dev_warn(dev, "no mode set in DT, guess shutdown (%d)\n", ret);
		mode = TPS6105X_MODE_SHUTDOWN;
	}

	pd->regulator_data = of_get_regulator_init_data(dev, of_node);
	pd->mode = mode;
...
static struct tps6105x_platform_data *
tps6105x_get_platform_data(struct device *dev)
{
	if (dev->platform_data)
		return dev->platform_data;
	else
		return tps6105x_i2c_parse_dt_pdata(dev);
}
			...
			const struct i2c_device_id *id)
{
	...
	pdata = tps6105x_get_platform_data(&client->dev);
	if (!pdata) {
...
static struct i2c_driver tps6105x_driver = {
	.driver = {
		.name	= "tps6105x",
		.owner  = THIS_MODULE,
		.of_match_table = of_match_ptr(tps6105x_dt_match),
	},
